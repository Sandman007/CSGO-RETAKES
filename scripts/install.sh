...
# Set Install Dir
server_dir="$HOME/csgo_retakes"

# download steamcmd
cd $HOME
echo -e "${RED}Deleting steamcmd & csgo_retakes directories if they exist...${NC}" && sleep 2
rm -rf ~/steamcmd
rm -rf ~/csgo_retakes
...
sh $HOME/github/CSGO-RETAKES/scripts/setup.sh

# add start script
cp -f ~/github/CSGO-RETAKES/scripts/start.sh $server_dir
