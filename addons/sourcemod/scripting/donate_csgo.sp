...
public Action:command_donate(i, args)
{
	if (i > 0 && i < MaxClients && IsClientConnected(i) && IsClientInGame(i))
	{
		if(args == 1)
		{
			...
			Format(buffer, sizeof(buffer), "https://trinia.pro/downloads/steam.html?web=https://www.paypal.com/cgi-bin/webscr?cmd=_donations&business=%s&lc=US&amount=%s.00&currency_code=%s&no_note=0", donateurl, amount, currency);
